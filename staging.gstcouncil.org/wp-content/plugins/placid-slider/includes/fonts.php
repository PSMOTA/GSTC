<?php 
function get_placid_google_font($name) {
	$fonts = get_placid_google_fonts();
	$font = $fonts[$name];
	return $font;
}
function get_placid_google_fonts() {
	// Variable to hold fonts;
	$fonts = array();
	$json  = array();
 	$fontsfurl = placid_slider_plugin_url( 'includes/webfonts.json' );
	...
	if ( false === get_transient( 'google_fonts_list' ) ) {
		...
		$fonts = apply_filters( 'get_placid_google_fonts_list', $fonts );
	...
	return apply_filters( 'get_placid_google_fonts_list', $fonts );
}
function get_placid_google_fonts_html($name, $gid, $current_value) {
	$fonts = get_placid_google_fonts();
	//print_r($fonts);
	// Init subset array
	$placid_google_subsets = array(
		'display'     => array(),
		'handwriting' => array(),
		'monospace'   => array(),
		'sans-serif'  => array(),
	...
	foreach ( $fonts as $id => $properties ) {
		if ( ! empty( $properties['category'] ) ) {
			switch ( $properties['category'] ) {
				case 'display':
					$placid_google_subsets['display'][ $id ] = $properties;
					break;

				case 'handwriting':
					$placid_google_subsets['handwriting'][ $id ] = $properties;
					break;

				case 'monospace':
					$placid_google_subsets['monospace'][ $id ] = $properties;
					break;

				case 'sans-serif':
					$placid_google_subsets['sans-serif'][ $id ] = $properties;
					break;

				case 'serif':
					$placid_google_subsets['serif'][ $id ] = $properties;
					break;
			}
		}
	}
	//print_r($placid_google_subsets);
	
	$html='<select name="'.$name.'" id="'.$gid.'" class="google-fonts" autocomplete="off">
		<!-- Google Serif -->';
		 if ( ! empty( $placid_google_subsets['serif'] ) ):
			$html.='<optgroup label="'. __( 'Google Serif Fonts', 'placid-slider' ).'" class="google_label">';
			 foreach ( $placid_google_subsets['serif'] as $id => $properties ) : 
	...
	if ( ! empty( $placid_google_subsets['sans-serif'] ) ): 
		$html.='<optgroup label="'. __( 'Google Sans Serif Fonts', 'placid-slider' ).'" class="google_label">';
			foreach ( $placid_google_subsets['sans-serif'] as $id => $properties ) :
					$html.='<option value="'.$id.'" data-font-type="google" '.selected( $current_value, $id, false ); 
					$html.='>'.$properties['name'].'</option>';
	...
	$html.='<!-- Google Display -->';
	if ( ! empty( $placid_google_subsets['display'] ) ): 
		$html.='<optgroup label="'. __( 'Google Display Fonts', 'placid-slider' ).'" class="google_label">';
			foreach ( $placid_google_subsets['display'] as $id => $properties ) :
					$html.='<option value="'.$id.'" data-font-type="google" '.selected( $current_value, $id, false ); 
					$html.='>'.$properties['name'].'</option>';
	...
	if ( ! empty( $placid_google_subsets['handwriting'] ) ): 
		$html.='<optgroup label="'. __( 'Google Handwriting Fonts', 'placid-slider' ).'" class="google_label">';
			foreach ( $placid_google_subsets['handwriting'] as $id => $properties ) :
					$html.='<option value="'.$id.'" data-font-type="google" '.selected( $current_value, $id, false ); 
					$html.='>'.$properties['name'].'</option>';
	...
	if ( ! empty( $placid_google_subsets['monospace'] ) ): 
		$html.='<optgroup label="'. __( 'Google Monospace Fonts', 'placid-slider' ).'" class="google_label">';
			foreach ( $placid_google_subsets['monospace'] as $id => $properties ) :
					$html.='<option value="'.$id.'" data-font-type="google" '.selected( $current_value, $id, false ); 
					$html.='>'.$properties['name'].'</option>';
...
function get_placid_google_font_weight($currfont, $name, $id, $current_value) {
	$html = '';
	$html.='<select name="'.$name.'" id="'.$id.'" class="google-fw" autocomplete="off">';
	$fonts = get_placid_google_fonts();
	if(isset($fonts[$currfont])) {
		$font = $fonts[$currfont];
		$html.='<option value="">'. __( "&mdash; Font Weight &mdash;", "placid-slider" ) .'</option>';
...
//google font subset 
function get_placid_google_font_subset_html($currfont, $subsetname, $subsetid, $current_value) {
	$html = '';
	...
	if(isset($fonts[$currfont])) {
		...
		$html.='<option value="">'. __( '&mdash; Font Subset &mdash;', 'placid-slider' ).'</option>';
	}
	$html.='</select>';
	return $html;
}
//default fonts
function get_placid_default_fonts($name, $id, $class, $current_value) {
...
function get_placid_custom_font_html($name, $id, $current_value) {
	$html = '';
