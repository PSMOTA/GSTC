...
class placid_update_class
{
    ...
    { 
        ...
        if (version_compare($this->current_version, $remote_version, '<')) {
			...
			if(isset($placid_license_key) && !empty($placid_license_key))
				$obj->package = 'http://slidervilla.com/store/receipt/?duid='.$placid_license_key;
    ...
    { 
        if ( isset( $arg->slug ) and ($arg->slug === $this->slug or $arg->slug === $this->plugin_slug) ) {
            $placid_license_key=get_option('placid_license_key');
			$information = $this->placid_getRemote_information();
			if(isset($placid_license_key) && !empty($placid_license_key)){
				$information = (array)$information;
				$information['download_link']='http://slidervilla.com/store/receipt/?duid='.$placid_license_key;
