...
function placid_sslider_image_by_the_post_thumbnail( $args = array() ) {
	...
	$fimage = wp_get_attachment_image_src( $post_thumbnail_id, 'full' );
	...
	return array( 'url' => $image[0], 'post_thumbnail_id' => $post_thumbnail_id, 'alt' => $alt, 'furl' => $fimage[0] );
}
...
function placid_sslider_image_by_attachment( $args = array() ) {
	...
	if ( empty( $attachments ) && $args['size'] ) {
		if ( 'attachment' == get_post_type( $args['post_id'] ) ) {
			...
			$fimage = wp_get_attachment_image_src( $args['post_id'], 'full' );
	...
	foreach ( $attachments as $id => $attachment ) {
		if ( ++$i == $args['order_of_image'] ) {
			...
			$fimage = wp_get_attachment_image_src( $id, 'full' );
	...
	return array( 'url' => $image[0], 'alt' => $alt, 'furl' => $fimage[0] );
}
...
function placid_sslider_display_the_image( $args = array(), $image = false ) {
	...
	if($imglink=='1' and $permalink!='') $permalink=isset($image['furl'])?$image['furl']:$image['url'];
...
//Image Cropping
	if($timthumb=='1'){
	  ...
	  $html = '<img src="'.bfi_thumb( $image['url'], $params ).'" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" class="' . esc_attr( $class ) . '"' . $width . $height . $style .' />';	  
    }
	else{
	...
	if ( $link_to_post ) {
		...
		$html = '<a class="placid_thumb_anchor" href="' . get_permalink( $post_id ) . '" '.$image_title.' '.$a_attr.'>' . $html . '</a>';}
	else {
	    if($permalink!='') {
	      $image_title=($image_title_text=='1')?( 'title="' . esc_attr( apply_filters( 'the_title', get_post_field( 'post_title', $post_id ) ) ) . '" '):'';
