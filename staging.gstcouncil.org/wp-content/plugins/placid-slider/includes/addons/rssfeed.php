...
function placid_carousel_posts_on_slider_rssfeed($max_posts='5', $offset=0, $out_echo = '1', $set='',$feedurl='',$data=array() ) {
   	$r_array=array();
	...
	if(isset($placid_slider_curr['rand']) ){
		$rand = $placid_slider_curr['rand'];
	}
	$src=isset($data['src'])?$data['src']:'';
	if($src=='smugmug'){		
		$feedurl = placidrssfeed_decode_entities($feedurl);
...
function get_placid_slider_feed($args='') {
	...
	$slider_handle='placid_slider_f'.$id;
	...
	$data['src']=$src;
	$data['size']=$size;
	$r_array = placid_carousel_posts_on_slider_rssfeed($placid_slider_curr['no_posts'], $offset, '0', $set, $feedurl, $data); 
	get_global_placid_slider($slider_handle,$r_array,$placid_slider_curr,$set,$echo='1',$data);
} 

function return_placid_slider_rssfeed($set='', $offset=0, $feedurl='', $default_image='', $image_class='', $id='1',$feed='', $order='0', $content='', $media='1', $title='', $src='', $size='') {
	$slider_html='';
	$default_placid_slider_settings=get_placid_slider_default_settings();
	// If setting set is 1 then set to blank	
	if($set == '1') $set = '';
...
function placid_slider_rssfeed_shortcode($atts) {
	...
	return return_placid_slider_rssfeed($set,$offset,$feedurl,$default_image,$image_class,$id,$feed,$order,$content,$media,$title,$src,$size);
}
add_shortcode('placidfeed', 'placid_slider_rssfeed_shortcode');
//Convert the encoded html to normal HTML characters in feed url
function placidrssfeed_decode_entities($text, $quote_style = ENT_COMPAT){
