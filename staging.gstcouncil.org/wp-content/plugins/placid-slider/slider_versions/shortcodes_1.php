...
function return_placid_slider($slider_id='',$set='',$offset=0,$format='',$data=array()) {
...
function placid_slider_simple_shortcode($atts) {
	...
	if( $type == 0 || $type == 17  ) {
		...
		$slider_html=return_placid_slider($id,$set,$offset,$format);
	} elseif( $type == 1 ) {
		$catg_slug=$param_array['catg_slug'];
		$slider_html=return_placid_slider_category($catg_slug,$scounter,$offset);
	} elseif( $type == 2 ) {
		$slider_html=return_placid_slider_recent($scounter,$offset);
	} elseif( $type == 3 ) {
		$post_type='product';
		...
		$slider_html=return_placid_slider_woocommerce($post_type,$taxonomy,$term,$scounter,$offset,$product_id,$type);
	} elseif( $type == 4 ) {
		...
		$data['type'] = 'ecom';
		$slider_html=return_placid_slider_taxonomy($post_type,$taxonomy,$term,$scounter,$offset,'','',$data);
	} elseif( $type == 5 ) {
		$post_type='event';
		...
		$slider_html=return_placid_slider_event($post_type, $term,$tags, $scounter, $offset,$scope);
	} elseif( $type == 6 ) {
		...
		$slider_html=return_placid_slider_event_calendar($post_type,$taxonomy,$term,$scounter,$offset,$tags,$evtype);
	} elseif( $type == 7 ) {
		...
		$data=array();
		$data['author']=$author;
		$slider_html=return_placid_slider_taxonomy($post_type,$taxonomy,$term,$scounter,$offset,$show,$operator,$data);
	} elseif( $type == 8 ) {
		...
		$slider_html=return_placid_slider_rssfeed($scounter,$offset,$feedurl,$default_image,$image_class,$rid,$feed,$order,$content,$media,$title,$src,$size);
	} elseif( $type == 9 ) {
		$paid=isset($param_array['postattch-id'])?$param_array['postattch-id']:'';
		$slider_html=return_placid_slider_attachments($scounter,$offset,$paid);
	} elseif( $type == 10 ) {
		...
		$slider_html=return_placid_slider_nggallery($gallery_id,$anchor,$scounter,$offset);
			
	} elseif( $type == 11 ) {
		$type='playlist';
		$val = isset($param_array['yt-playlist-id'])?$param_array['yt-playlist-id']:'';
		$slider_html=return_placid_slider_youtube($type,$val,$scounter,$offset);
		...
		$slider_html=return_placid_slider_vimeo($type,$val,$scounter,$offset);
			
	} elseif( $type == 14 ) { 
		$type='page';
		$album = isset($param_array['fb-album'])?$param_array['fb-album']:'';
		$slider_html=return_placid_slider_facebook($type,$album,$scounter,$offset);
			
	} elseif( $type == 15 ) {
		$type='';
		$username = isset($param_array['user-name'])?$param_array['user-name']:'';
		$slider_html=return_placid_slider_instagram($type,$username,$scounter,$offset);
			
	} elseif( $type == 16 ) {
		$type=isset($param_array['flickr-type'])?$param_array['flickr-type']:'';
		$fid = isset($param_array['fl-id'])?$param_array['fl-id']:'';
		$slider_html=return_placid_slider_flickr($type,$fid,$scounter,$offset);
			
	} elseif( $type == 18 ) {
		$feature=isset($param_array['feature'])?$param_array['feature']:'';
		$username = isset($param_array['pxuser'])?$param_array['pxuser']:'';
		$slider_html=return_placid_slider_500px($feature,$username,$scounter,$offset);
	}
		...
		if(isset($gplacid_slider['editlink']) && $gplacid_slider['editlink'] == "1"){
			if ( !is_admin() && is_user_logged_in() && count($result) > 0 ) {
				if ( current_user_can('manage_options') ) {
					$editurl = placid_sslider_admin_url( array( 'page' => 'placid-slider-easy-builder' ) ).'&id='.$id;
...
function placid_slider_category_shortcode($atts) {
	...
	return return_placid_slider_category($catg_slug,$set,$offset);
}
...
function placid_slider_recent_shortcode($atts) {
	...
	return return_placid_slider_recent($set,$offset);
}
