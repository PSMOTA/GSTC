  ...
  public static function printPDF($contribIDs, &$params, $contactIds) {
    ...
    foreach ($invoiceElements['details'] as $contribID => $detail) {
      ...
      if (!empty($relatedContact['individual_id'])) {
        list($mainDisplayName, $mainEmail) = CRM_Contact_BAO_Contact_Location::getEmailDetails($relatedContact['individual_id']);
        $isMainContact = true;
      } else if (!empty($ids['membership']) && !empty($contribution->contact_id)) {
        ...
        if ($contactType == 'Organization') {
          ...
          $paramsRelationship = array(1 => array($contribution->contact_id, 'Integer'));
          $mainContactID = CRM_Core_DAO::singleValueQuery($sql, $paramsRelationship);      
          if (!empty($mainContactID)) {
            list($mainDisplayName, $mainEmail) = CRM_Contact_BAO_Contact_Location::getEmailDetails($mainContactID);
            $isMainContact = true;
          }
        }
      }
      //CRM_Core_Error::debug_var('mainDisplayName', $mainDisplayName);
      ...
      $tplParams = array(
        ...
        'display_name' => !empty($mainDisplayName)? $mainDisplayName : $contribution->_relatedObjects['contact']->display_name,
        'lineItem' => $lineItem,
        'dataArray' => $dataArray,
        ...
        'domain_postal_code_suffix' => CRM_Utils_Array::value('postal_code_suffix', CRM_Utils_Array::value('1', $locationDefaults['address'])),
      ...
      elseif ($contribution->_component == 'contribute') {
        ...
        $sendTemplateParams['toEmail'] = $isMainContact?$mainEmail:$email;
