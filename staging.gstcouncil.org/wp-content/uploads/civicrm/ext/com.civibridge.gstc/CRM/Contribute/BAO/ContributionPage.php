...
class CRM_Contribute_BAO_ContributionPage extends CRM_Contribute_DAO_ContributionPage {
  ...
  public static function sendMail($contactID, $values, $isTest = FALSE, $returnMessageText = FALSE, $fieldTypes = NULL) {
    ...
    $email = NULL;
    unset($values['custom_pre_id'] );
    unset($values['custom_post_id']);
    ...
    if (!empty($values['is_email_receipt']) || !empty($values['onbehalf_dupe_alert']) ||
      ...
      if (array_key_exists('related_contact', $values)) {
        ...
        // overwrite email sending, only send email to main contact and not to organzation contact
        list($displayName, $email) = CRM_Contact_BAO_Contact_Location::getEmailDetails($values['related_contact']);
        $tplParams['email'] = $email;
