  ...
  protected function assignPaymentRelatedVariables() {
    try {
      if ($this->_contactID) {
        ...
        if ($this->_id) {
          $params = array('contribution_id' => $this->_id, 'contact_id' => $this->_contactID);
          $mainContactID = CRM_Utils_Gstc::getMainContact($params);
          if ($mainContactID) {
            list($this->userDisplayName, $this->userEmail) = CRM_Contact_BAO_Contact_Location::getEmailDetails($mainContactID);
          }
        }
