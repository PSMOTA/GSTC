      ...
      if (is_array($memberships)) {
        foreach ($memberships as $membership) {
          if ($membership) {
            ...
            //++++++++++++++++++++++++++++++++
            // Update Activity status for related membership
            $membershipRelated = new CRM_Member_DAO_Membership();
            $membershipRelated->owner_membership_id = $membership->id;
            $membershipRelated->find();
            $scheduledActivityID = '';
            while ($membershipRelated->fetch()) {
              foreach (array('Membership Signup', 'Membership Renewal') as $activityType) {
                $scheduledActivityID = CRM_Utils_Array::value('id',
                  civicrm_api3('Activity', 'Get',
                    array(
                      'source_record_id' => $membershipRelated->id,
                      'activity_type_id' => $activityType,
                      'status_id' => 'Scheduled',
                      'options' => array(
                        'limit' => 1,
                ...
                // 2. OR Create renewal activity scheduled if its membership renewal will be paid later
                if ($scheduledActivityID) {
                  CRM_Activity_BAO_Activity::addActivity($membershipRelated, $activityType, $membershipRelated->contact_id, array('id' => $scheduledActivityID));
                  break;
                }
              }
            }

            $membershipRelated->free();
            //#######################################END@@@@@@@@@@@@@@@@@@@@@@@@@@2
