...
";
      ...
      if ($allStatus[$activity->status_id] ==  'Pending' && $activity->is_pay_later) {
        $activityParamsExtra['status_id'] = CRM_Core_OptionGroup::getValue('activity_status', 'Scheduled', 'name');
      }
    }
    ...
    // Overwrite activity status if needed
    $activityParams = array_merge($activityParams, $activityParamsExtra, $params);
    ...
    if ($targetContactID) {
      ...
      if (empty($activityParams['source_contact_id'])) {
        $activityParams['source_contact_id'] = $targetContactID;
      }
    }
